...
func Test_startBurnCpu(t *testing.T) {
	...
	burnCPU.RunBurnCpu = func(ctx context.Context, cpuCount int, pidNeeded bool, processor string) int {
		return 25233
	}
	burnCPU.BindBurnCpuByCpuSet = func(cgctrl cgroups.Cgroup, cpulist string) {}
	burnCPU.CheckBurnCpu = func(ctx context.Context) {}
	burnCPU.CGroupNew = func(int, int) cgroups.Cgroup { return new(CgroupMock) }
...
func Test_runBurnCpu_failed(t *testing.T) {
	...
	burnCPU.RunBurnCpu(context.Background(), as.cpuCount, as.pidNeeded, as.processor)
	if exitCode != 1 {
