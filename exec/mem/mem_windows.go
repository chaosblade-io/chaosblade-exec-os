...
func (ce *memExecutor) Exec(uid string, ctx context.Context, model *spec.ExpModel) *spec.Response {
	...
	ce.start(ctx, memPercent, memReserve, memRate, burnMemModeStr, includeBufferCache, ce.channel)
	return spec.Success()
}
...
func burnMemWithCache(ctx context.Context, memPercent, memReserve, memRate int, burnMemMode string, includeBufferCache bool, cl spec.Channel) {
	filedir := path.Join(util.GetProgramPath(), dirName)
	if !exec.CheckFilepathExists(ctx, nil, filedir) {
		if err := os.Mkdir(filedir, 0666); err != nil {
			log.Fatalf(ctx, "mkdir `%s` failed, err, %v", filedir, err)
			return
		}
	}
	...
	for range tick {
		...
		if expectMem > 0 {
			...
			response := cl.Run(ctx, "dd", fmt.Sprintf("if=NUL of=%s bs=1M count=%d", nFilePath, fillMem))
			if !response.Success {
				log.Fatalf(ctx, "burn mem with cache err, %v", response.Err)
			}
			fileCount++
		}
	}
...
// start burn mem
func (ce *memExecutor) start(ctx context.Context, memPercent, memReserve, memRate int, burnMemMode string, includeBufferCache bool, cl spec.Channel) {
